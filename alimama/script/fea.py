...
for raw_line in open(sys.argv[1]):
  ...
  if user_id in user_dict:
    if user_dict[user_id]["show"] < 10:
      fp_w.write("%s\001%s\001"%(user_dict[user_id]["show"], user_dict[user_id]["conv"]))
    else:
      fp_w.write("%s\001%s\001"%(10, user_dict[user_id]["conv"]))
  else:
    fp_w.write("0\0010\001")
  #write other fea
  ...
  if review_position_rate not in review_position_rate_dict:
    ...
    fp_w.write("%s\001"%review_position_rate_dict[review_position_rate]["conv"])
  
  if star_level not in shop_on_star_level:
    fp_w.write("0\0010\001")
  else:
    if shop_id not in shop_on_star_level[star_level]:
      fp_w.write("0\0010\001")
    else:
      fp_w.write("%s\001"%shop_on_star_level[star_level][shop_id]["show"])
      fp_w.write("%s\001"%shop_on_star_level[star_level][shop_id]["conv"])
  if star_level not in star_level_dict:
    fp_w.write("0\0010\n")
  else:
    fp_w.write("%s\001"%star_level_dict[star_level]["show"])
    fp_w.write("%s\n"%star_level_dict[star_level]["conv"])
  ...
  if star_level not in shop_on_star_level:
    shop_on_star_level[star_level] = {}
  if shop_id not in shop_on_star_level[star_level]:
    shop_on_star_level[star_level][shop_id] = {"show":0, "conv":0}
  shop_on_star_level[star_level][shop_id]["show"] += 1
  shop_on_star_level[star_level][shop_id]["conv"] += int(label)
  if star_level not in star_level_dict:
    star_level_dict[star_level] = {"show":0, "conv":0}
  star_level_dict[star_level]["show"] += 1
  star_level_dict[star_level]["conv"] += int(label)
